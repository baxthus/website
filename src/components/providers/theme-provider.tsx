...
const Theme = ({
  ...
  const applyAttributesToDOM = useCallback(
    (resolved: string) => {
      ...
      for (const attr of attributeList) {
        if (attr === 'class') applyClassAttribute(name, attrValues);
        else if (attr.startsWith('data-')) applyDataAttribute(attr, name);
      }
    },
  ...
  const setTheme = useCallback(
    (newValue: SetStateAction<string>) => {
      const newTheme = typeof newValue === 'function' ? newValue(themeState ?? '') : newValue;
  ...
  const handleMediaQuery = useCallback(
    (_event: MediaQueryListEvent | MediaQueryList) => {
      if (themeState === 'system' && enableSystem && !forcedTheme) applyTheme('system');
    },
    [applyTheme, enableSystem, forcedTheme, themeState],
  );
  ...
  const providerValue = useMemo(
    ...
    [themeState, forcedTheme, enableSystem, themes, setTheme],
  );
